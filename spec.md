...
每天北京时间 09:00 获取 GitHub Trending Daily 全语言总榜前 20 个仓库，筛选与
Agent 应用生态相关的项目，生成可公开的中文 Markdown 研究简报并由 GitHub Actions
直接提交到默认分支。

输出数量允许为 0–20，不从榜外补足。每个结论必须能追溯到 GitHub 原始数据、确定性
规则、人工名单和（如有）Qwen 结构化判定。任一必要步骤失败时不得发布不完整结果，
同日成功重跑原子覆盖已有文件。
...
1. 请求 `https://github.com/trending?since=daily` 并按页面顺序解析前 20 个仓库；
   不足 20 个结构完整的卡片即失败。
...
`data/YYYY-MM-DD.json` 永久保存 schema 版本、北京时间运行信息、来源、模型、全部
20 个候选的排名和 GitHub 元数据、README 哈希、规则证据、人工/LLM/最终判定及历史
统计；不保存完整 README。
...
首版验收要求手动 Action 产生恰好 20 个候选判定；日报只展示 `included=true` 项并保持
原始排名；同日只有一个归档版本；失败重跑不改变旧内容；零结果正常发布；离线测试、
静态检查和真实网络 smoke test全部通过。
